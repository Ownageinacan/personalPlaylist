...
//	private List<Artist> artistList;
	...
	// the DB currently consists of lists of songs and playlists
	public FakeDatabase() {
		songList = new ArrayList<Song>();
		playList = new ArrayList<Playlist>();
//		albumList = new ArrayList<Album>();		// ADD ALBUM/ARTIST?
...
//			albumList.addAll(InitialData.getAlbums());	// ADD ALBUM/ARTIST?
//			artistList.addAll(InitialData.getArtists());
	...
	@Override
	public List<Playlist> findAllPlaylists()	//return a list of playlists (NO SONGS)
	{
		return playList;
	}
	
//	public List<Album> findAllAlbums()	// ADD ALBUM/ARTIST?
//	{
//		return albumList;
//	}
//
...
//	}
	...
	@Override
	//TODO: GET THIS WORKING USING BLACK MAGIC, if something's wrong?; Catch a "no input" exception
	//Returns a list of songs in a playlist
	public List<Pair<Song,Playlist>> findAllSongInPlaylist(String playlist)	//Return songs within a given playlist (TEMP)
	{
		...
		for(Playlist pl : playList){
			if(pl.getTitle().equals(playlist)){	//See: getTitle method in playlist class
				...
				for(Song sng : songList){	//for each song find a song by certain Id?
					Song song = findSongBySongId(sng.getSongId());
					result.add(new Pair<Song, Playlist>(song, pl)); //add to the result as a pair
				}
			}
		}
		return result;
